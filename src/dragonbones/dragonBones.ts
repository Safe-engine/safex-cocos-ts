...
dragonBones.WorldClock = cc.Class.extend({
  ...
  remove: function (animatable) {
    const index = this._animatableList.indexOf(animatable);
    if (index >= 0) {
  ...
  advanceTime: function (passedTime) {
    ...
    let len = this._animatableList.length;
    if (len == 0) {
      return;
    }
...
dragonBones.Animation = cc.Class.extend({
  ...
  getIsComplete: function () {
    if (this._lastAnimationState) {
      ...
      const l = this._animationStateList.length;
      for (let i = 0; i < l; i++) {
  ...
  setAnimationDataList: function (animationDataList) {
    ...
    const l = this._animationDataList.length;
    for (let i = 0; i < l; i++) {
  ...
  clear: function () {
    ...
    let l = this._animationStateList.length;
    let i;
    for (i = 0; i < l; i++) {
      dragonBones.AnimationState.returnObject(this._animationStateList[i]);
    }
  ...
  gotoAndPlay: function (animationName, fadeInTime, duration, playTimes, layer, group, fadeOutMode, pauseFadeOut, pauseFadeIn) {
    ...
    let animationData;
    let i = this._animationDataList.length;
    while (--i >= 0) {
    ...
    const slotList = this._armature.getSlots();
    let slot;
    let childArmature;
    i = slotList.length;
    while (i--) {
  ...
  gotoAndStop: function (animationName, time, normalizedTime, fadeInTime, duration, layer, group, fadeOutMode) {
    ...
    let animationState = this.getState(animationName, layer);
    if (!animationState) {
  ...
  getState: function (name, layer) {
    ...
    let animationState;
    let i = this._animationStateList.length;
    while (--i >= 0) {
      animationState = this._animationStateList[i];
  ...
  removeState: function (animationState) {
    const index = this._animationStateList.indexOf(animationState);
...
dragonBones.AnimationState = cc.Class.extend({
  ...
  addMixingTransform: function (timelineName, recursive) {
    ...
    if (recursive) {
      ...
      let bone;
      let boneName;
      const boneList = this._armature.getBones();
      let i = boneList.length;
      while (--i >= 0) {
        bone = boneList[i];
        boneName = bone.name;
  ...
  updateTimelineStates: function () {
    let timelineState;
    let i = this._timelineStateList.length;
    let len;
    while (--i >= 0) {
  ...
  addTimelineState: function (timelineName) {
    const bone = this._armature.getBone(timelineName);
    if (bone) {
      const len = this._timelineStateList.length;
      let i = 0;
      while (i < len) {
      ...
      const timelineState = dragonBones.TimelineState.borrowObject();
  ...
  removeTimelineState: function (timelineState) {
    const index = this._timelineStateList.indexOf(timelineState);
    if (index >= 0) {
  ...
  advanceFadeTime: function (passedTime) {
    let fadeStartFlg = false;
    let fadeCompleteFlg = false;

    if (this._fadeBeginTime >= 0) {
      let fadeState = this._fadeState;
  ...
  advanceTimelinesTime: function (passedTime) {
    ...
    let loopCompleteFlg = false;
    let isThisComplete = false;
    let currentPlayTimes = 0;
    let currentTime = this._time * 1000;//这里需要int型
    ...
    if (this._playTimes == 0) {
      ...
      const totalTimes = this._playTimes * this._totalTime;
    ...
    const progress = this._time * 1000 / this._totalTime;//存在问题

    const l = this._timelineStateList.length;
    let i = 0;
    let timeline;
    while (i < l) {
  ...
  updateMainTimeline: function (isThisComplete) {//自己修改过代码
    const frameList = this._clip.frameList;
    const l = frameList.length;
    if (l > 0) {
      let prevFrame;
      let currentFrame;
  ...
  hideBones: function () {
    let bone;
    for (let i = 0, l = this._clip.hideTimelineList.length; i < l; i++) {
...
dragonBones.AnimationState.returnObject = function (animationState) {
  const index = DBAnimationStatePool.indexOf(animationState);
  if (index < 0) {
...
dragonBones.AnimationState.clearObjects = function () {
  let i = DBAnimationStatePool.length;
  while (--i >= 0) {
...
dragonBones.TimelineState = cc.Class.extend({
  ...
  updateMultipleFrame: function (progress) {
    ...
    let currentTime = this._totalTime * progress;//这里需要int型
    let currentPlayTimes = 0;
    const playTimes = this._animationState.getPlayTimes();
    ...
    if (playTimes == 0) {
      ...
      const totalTimes = playTimes * this._totalTime;
      if (currentTime >= totalTimes) {
        currentTime = totalTimes;
    ...
    if (this._currentTime != currentTime) {
      ...
      let prevFrame;
      let currentFrame;
      const frameList = this._timeline.frameList;
      const l = frameList.length;
      let i;
      for (i = 0; i < l; ++i) {
        if (this._currentFrameIndex < 0) {
  ...
  updateToNextFrame: function (currentPlayTimes) {
    ...
    let nextFrameIndex = this._currentFrameIndex + 1;
    if (nextFrameIndex >= this._timeline.frameList.length) {
      nextFrameIndex = 0;
    }

    const currentFrame = this._timeline.frameList[this._currentFrameIndex];
    const nextFrame = this._timeline.frameList[nextFrameIndex];
  ...
  updateTween: function () {
    let progress = (this._currentTime - this._currentFramePosition) / this._currentFrameDuration;
    ...
    if (this._tweenTransform) {
      const currentTransform = currentFrame.transform;
      const currentPivot = currentFrame.pivot;
      if (this._animationState.additiveBlending) {
        //additive blending
  ...
  updateSingleFrame: function () {
    const currentFrame = this._timeline.frameList[0];
    this._bone.arriveAtFrame(currentFrame, this, this._animationState, false);
    this._isComplete = true;
    this._tweenTransform = false;
    this._tweenScale = false;
...
dragonBones.TimelineState.clearObjects = function () {
  let i = DBTimelineStatePool.length;
  while (--i >= 0) {
...
dragonBones.Armature = cc.Class.extend({
  ...
  getBone: function (boneName) {
    const len = this._boneList.length;
    let bone;
    for (let i = 0; i < len; ++i) {
  ...
  addBone: function (bone, parentBoneName) {
    if (parentBoneName === undefined) {
      ...
      const boneParent = this.getBone(parentBoneName);
  ...
  getSlot: function (slotName) {
    const len = this._slotList.length;
    let slot;
    for (let i = 0; i < len; ++i) {
  ...
  addSlot: function (slot, parentBoneName) {
    const bone = this.getBone(parentBoneName);
    if (bone) {
      bone.addChild(slot);
    } else {
  ...
  advanceTime: function (passedTime) {
    ...
    const isFading = this._animation._isFading;
    ...
    const len = this._eventDataList.length;
    let event;
    if (len > 0) {
      for (i = 0; i < len; ++i) {
...
dragonBones.Bone = dragonBones.Object.extend({
  ...
  addChild: function (object) {
    ...
    const bone = (object instanceof dragonBones.Bone) ? object : null;
    const slot = (object instanceof dragonBones.Slot) ? object : null;
  ...
  update: function (needUpdate) {
    ...
    if (this._parent) {
      const x = this.origin.x + this.offset.x + this._tween.x;
      const y = this.origin.y + this.offset.y + this._tween.y;
  ...
  arriveAtFrame: function (frame, timelineState, animationState, isCross) {
    const displayControl = animationState.displayControl && (!this.displayController || this.displayController == animationState.name);

    let l;
    let i;
    let slot;
    if (displayControl) {
      const displayIndex = frame.displayIndex;
      l = this._slotList.length;
      for (i = 0; i < l; ++i) {
        slot = this._slotList[i];
  ...
  blendingTimeline: function () {
    ...
    if (i == 1) {
      ...
      let prevLayer = this._timelineStateList[i - 1]._animationState.getLayer();
      let currentLayer = 0;
      let weigthLeft = 1;
...
dragonBones.Slot = dragonBones.Object.extend({
  ...
  setDisplayList: function (displayList, disposeExisting) {
    ...
    const displayIndexBackup = this._displayIndex;
    this._displayIndex = -1;
  ...
  update: function () {
    ...
    const x = this.origin.x + this.offset.x + this._parent._tweenPivot.x;
    const y = this.origin.y + this.offset.y + this._parent._tweenPivot.y;
    const parentMatrix = this._parent.globalTransformMatrix;
    this.globalTransformMatrix.tx = this.global.x = parentMatrix.a * x + parentMatrix.c * y + parentMatrix.tx;
    this.globalTransformMatrix.ty = this.global.y = parentMatrix.d * y + parentMatrix.b * x + parentMatrix.ty;
  ...
  changeDisplay: function (displayIndex = 0) {
    if (displayIndex < 0) {
  ...
  updateSlotDisplay: function (disposeExisting) {
    ...
    const display = this._displayList[this._displayIndex];
...
dragonBones.EventData.borrowObject = function (eventType) {
  ...
  const eventData = DBEventDataPool.pop();
...
dragonBones.BaseFactory = cc.Class.extend({
  ...
  removeDragonBonesData: function (name, disposeData) {
    ...
    let data = this._dragonBonesDataMap[name];
    if (data) {
      if (disposeData) {
        data.dispose();
      }
  ...
  addTextureAtlas: function (textureAtlas, name) {
    const key = textureAtlas.textureAtlasData.name || name;
  ...
  removeTextureAtlas: function (name, disposeData) {
    ...
    let data = this._textureAtlasMap[name];
    if (data) {
      if (disposeData) {
        data.dispose();
      }
  ...
  buildArmature: function (armatureName, skinName, animationName, dragonBonesName, textureAtlasName) {
    ...
    let animationArmatureData = null;
    let skinData = null;
    let skinDataCopy = null;
    let key;
    ...
    if (!armatureData) {
      const searchType = (dragonBonesName == null ? dragonBones.AutoSearchType.AST_ALL : (this.autoSearchDragonBonesData ? dragonBones.AutoSearchType.AST_AUTO : dragonBones.AutoSearchType.AST_NONE));
    ...
    const armature = this.generateArmature(armatureData);
  ...
  getTextureDisplay: function (textureName, textureAtlasName, displayData) {
    ...
    if (!textureData) {
      const searchType = (textureAtlasName == null ? dragonBones.AutoSearchType.AST_ALL : (this.autoSearchTexture ? dragonBones.AutoSearchType.AST_AUTO : dragonBones.AutoSearchType.AST_NONE));
    ...
    if (!displayData) {
      const data = this._dragonBonesDataMap[textureAtlas.textureAtlasData.name];
      if (data) {
        const dragonBonesData = data;

        for (let i = 0, l1 = dragonBonesData.armatureDataList.length; i < l1; ++i) {
          for (let j = 0, l2 = dragonBonesData.armatureDataList[i].skinDataList.length; j < l2; ++j) {
            for (let k = 0, l3 = dragonBonesData.armatureDataList[i].skinDataList[j].slotDataList.length; k < l3; ++k) {
              for (let m = 0, l4 = dragonBonesData.armatureDataList[i].skinDataList[j].slotDataList[k].displayDataList.length; m < l4; ++m) {
  ...
  buildBones: function (armature, armatureData) {
    let boneData;
    let bone;
    for (let i = 0, l = armatureData.boneDataList.length; i < l; ++i) {
  ...
  buildSlots: function (armature, armatureData, skinData, skinDataCopy) {
    ...
    for (let i = 0, l1 = skinData.slotDataList.length; i < l1; ++i) {
      ...
      for (let j = 0, l2 = slotData.displayDataList.length; j < l2; ++j) {
        const displayData = slotData.displayDataList[j];

        switch (displayData.type) {
          case dragonBones.DisplayType.DT_ARMATURE:
            {
              let displayDataCopy = null;

              if (skinDataCopy) {
                const slotDataCopy = skinDataCopy.getSlotData(slotData.name);
                if (slotDataCopy) {
                  displayDataCopy = slotDataCopy.displayDataList[i];
                }
              }

              const currentDragonBonesDataName = this._currentDragonBonesDataName;
              const currentTextureAtlasName = this._currentTextureAtlasName;
              const childArmature = this.buildArmature(displayData.name, '', displayDataCopy ? displayDataCopy.name : '', currentDragonBonesDataName, currentTextureAtlasName);
          ...
          case dragonBones.DisplayType.DT_IMAGE:
            {
              const display = this.getTextureDisplay(displayData.name, this._currentTextureAtlasName, displayData);
...
dragonBones.Matrix = cc.Class.extend({
  ...
  invert: function () {
    ...
    const tx0 = this.tx;
    const ty0 = this.ty;
    const determinant = 1 / (a0 * d0 - b0 * c0);
...
dragonBones.Transform = cc.Class.extend({
  ...
  transformWith: function (parent) {
    const matrix = new dragonBones.Matrix();
...
dragonBones.AnimationData = dragonBones.Timeline.extend({
  ...
  getTimeline: function (timelineName) {
    for (let i = 0, l = this.timelineList.length; i < l; ++i) {
  ...
  dispose: function () {
    ...
    for (const timelineName in this.timelineList) {
...
dragonBones.ArmatureData = cc.Class.extend({
  ...
  dispose: function () {
    let i = this.boneDataList.length;
    while (i--) {
  ...
  getSkinData: function (skinName) {
    ...
    let i = this.skinDataList.length;
    while (i--) {
  ...
  getAnimationData: function (animationName) {
    let i = this.animationDataList.length;
    while (i--) {
  ...
  sortBoneDataList: function () {
    ...
    const helpArray = [];
    while (i--) {
      const boneData = this.boneDataList[i];
      let level = 0;
...
dragonBones.DragonBonesData = cc.Class.extend({
  ...
  dispose: function () {
    let i = this.armatureDataList.length;
    while (i--) {
...
dragonBones.SkinData = cc.Class.extend({
  ...
  dispose: function () {
    let i = this.slotDataList.length;
    while (i--) {
...
dragonBones.SlotData = cc.Class.extend({
  ...
  dispose: function () {
    let i = this.displayDataList.length;
    while (i--) {
...
dragonBones.BaseDataParser.transformArmatureData = function (armatureData) {
  let i = armatureData.boneDataList.length;
  let boneData;
  let parentBoneData;
  while (--i >= 0) {
    boneData = armatureData.boneDataList[i];
...
dragonBones.BaseDataParser.transformArmatureDataAnimations = function (armatureData) {
  for (let i = 0, l = armatureData.animationDataList.length; i < l; ++i) {
...
dragonBones.BaseDataParser.transformAnimationData = function (animationData, armatureData) {
  const skinData = armatureData.getSkinData(null);
  ...
  for (let i = 0, l = armatureData.boneDataList.length; i < l; ++i) {
    ...
    if (skinData) {
      for (let j = 0, len = skinData.slotDataList.length; j < len; ++j) {
    ...
    for (let k = 0, l1 = timeline.frameList.length; k < l1; ++k) {
...
dragonBones.BaseDataParser.setFrameTransform = function (animationData, armatureData, boneData, frame) {
  ...
  let parentData = armatureData.getBoneData(boneData.parent);

  if (parentData) {
    let parentTimeline = animationData.getTimeline(parentData.name);

    if (parentTimeline) {
      const parentTimelineList = [];
      ...
      const helpMatrix = new dragonBones.Matrix();
      const currentTransform = new dragonBones.Transform();
      let globalTransform = null;

      for (let i = parentTimelineList.length; i--;) {
        ...
        if (globalTransform) {
          ...
          const x = currentTransform.x + parentTimeline.originTransform.x + parentData.transform.x;
          const y = currentTransform.y + parentTimeline.originTransform.y + parentData.transform.y;
...
dragonBones.BaseDataParser.getTimelineTransform = function (timeline, position, retult, isGlobal) {
  ...
  let progress;
  let tweenEasing;
  for (let i = 0, l = timeline.frameList.length; i < l; ++i) {
    ...
    if (currentFrame.position <= position && currentFrame.position + currentFrame.duration > position) {
      if (i == timeline.frameList.length - 1 || position == currentFrame.position) {
        ...
        const nextFrame = timeline.frameList[i + 1];
        const currentTransform = isGlobal ? currentFrame.global : currentFrame.transform;
        const nextTransform = isGlobal ? nextFrame.global : nextFrame.transform;
...
dragonBones.ObjectDataParser = dragonBones.BaseDataParser.extend({
  ...
  parseTextureAtlasData: function (rawTextureAtlasData, scale) {
    ...
    const textureAtlasData = new dragonBones.TextureAtlasData();
    ...
    let textureData;
    const subTextureList = rawTextureAtlasData[DBConstValues.SUB_TEXTURE];
    for (const key in subTextureList) {
  ...
  parseDragonBonesData: function (rawDragonBonesData, scale = 1) {
    ...
    const dragonBonesData = new dragonBones.DragonBonesData();
    dragonBonesData.name = rawDragonBonesData[DBConstValues.A_NAME];

    let armatureData;
    const armatureList = rawDragonBonesData[DBConstValues.ARMATURE];
    for (const key in armatureList) {
  ...
  parseTextureData: function (textureXML) {
    const textureData = new dragonBones.TextureData();
    ...
    const frameWidth = this.getNumber(textureXML, DBConstValues.A_FRAME_WIDTH, 0) / this._textureScale;
    const frameHeight = this.getNumber(textureXML, DBConstValues.A_FRAME_HEIGHT, 0) / this._textureScale;
  ...
  parseArmatureData: function (armatureXML) {
    const armatureData = new dragonBones.ArmatureData();
    armatureData.name = armatureXML[DBConstValues.A_NAME];

    let key;

    const boneList = armatureXML[DBConstValues.BONE];
    let boneData;
    for (key in boneList) {
    ...
    const skinList = armatureXML[DBConstValues.SKIN];
    const slotsList = armatureXML[DBConstValues.SLOT];
    ...
    const animationList = armatureXML[DBConstValues.ANIMATION];
    let animationData;
  ...
  parseBoneData: function (boneXML) {
    const boneData = new dragonBones.BoneData();
    boneData.name = boneXML[DBConstValues.A_NAME];
    const parent = boneXML[DBConstValues.A_PARENT];
    if (parent) {
    ...
    const transformXML = boneXML[DBConstValues.TRANSFORM];
    if (transformXML) {
  ...
  parseSkinData: function (skinXML, slotsList) {
    const skinData = new dragonBones.SkinData();
    skinData.name = skinXML[DBConstValues.A_NAME];

    const slotList = skinXML[DBConstValues.SLOT];
    let slotData;
  ...
  parseSlotData: function (slotXML, slotsList = []) {
    const slotData = new dragonBones.SlotData();
    slotData.name = slotXML[DBConstValues.A_NAME];
    slotData.parent = slotXML[DBConstValues.A_PARENT] ||
      slotsList.find(slot => slot.name == slotData.name).parent;
    ...
    const displayList = slotXML[DBConstValues.DISPLAY];
    let displayData;
  ...
  parseDisplayData: function (displayXML) {
    const displayData = new dragonBones.DisplayData();
    ...
    const scalingGridXML = displayXML[DBConstValues.SCALING_GRID];
    if (scalingGridXML) {
    ...
    const transformXML = displayXML[DBConstValues.TRANSFORM];
    if (transformXML) {
  ...
  parseAnimationData: function (animationXML, armatureData) {
    const animationData = new dragonBones.AnimationData();
    ...
    const frameObjectList = animationXML[DBConstValues.FRAME];
    let frame;
    ...
    const timelineObjectList = animationXML[DBConstValues.TIMELINE];
    let timeline;
  ...
  parseTransformTimeline: function (timelineXML, duration) {
    const timeline = new dragonBones.TransformTimeline();
    ...
    const frameList = timelineXML[DBConstValues.FRAME];
    let frame;
    for (const key in frameList) {
  ...
  parseMainFrame: function (frameXML) {
    const frame = new dragonBones.Frame();
    this.parseFrame(frameXML, frame);
    return frame;
  },

  parseTransformFrame: function (frameXML) {
    const frame = new dragonBones.TransformFrame();
    this.parseFrame(frameXML, frame);
    ...
    const transformXML = frameXML[DBConstValues.TRANSFORM];
    if (transformXML) {
    ...
    const colorTransformXML = frameXML[DBConstValues.COLOR_TRANSFORM];
...
dragonBones.TextureAtlasData = cc.Class.extend({
  ...
  dispose: function () {
    for (let i = 0, l = this.textureDataList.length; i < l; ++i) {
...
dragonBones.DBCCArmature = dragonBones.Armature.extend({
  ...
  sortSlotsByZOrder: function () {
    ...
    let nShowCount = 0;
    const nDisplayChildrenCount = this._display.getChildrenCount();

    let slot;
    let slotDisplayNode;
    for (let i = 0, l = this._slotList.length; i < l; ++i) {
      slot = this._slotList[i];
...
dragonBones.DBCCFactory = dragonBones.BaseFactory.extend({
  ...
  loadDragonBonesData: function (dragonBonesFilePath, name) {
    const existDragonBonesData = this.getDragonBonesData(name);
    if (existDragonBonesData) {
      return existDragonBonesData;
    }

    const data = cc.loader.getRes(dragonBonesFilePath);
    ...
    const parser = new dragonBones.ObjectDataParser();
    const dragonBonesData = parser.parseDragonBonesData(data, scale);
  ...
  loadTextureAtlas: function (textureAtlasFile, name) {
    ...
    const existTextureAtlas = this.getTextureAtlas(name);
    ...
    const data = cc.loader.getRes(textureAtlasFile);
    ...
    const parser = new dragonBones.ObjectDataParser();
    const textureAtlas = new dragonBones.DBCCTextureAtlas();
    textureAtlas.textureAtlasData = parser.parseTextureAtlasData(data, scale);

    const pos = textureAtlasFile.lastIndexOf('/');

    if (-1 != pos) {
      const base_path = textureAtlasFile.substr(0, pos + 1);
  ...
  refreshTextureAtlasTexture: function (name) {
    let textureAtlas;
    for (const key in this._textureAtlasMap) {
      textureAtlas = this._textureAtlasMap[key];
  ...
  hasDragonBones: function (skeletonName, armatureName, animationName) {
    ...
    const dragonbonesData = this.getDragonBonesData(skeletonName);

    if (!dragonbonesData) { return false; }

    if (armatureName) {
      const armatureData = dragonbonesData.getArmatureData(armatureName);

      if (!armatureData) { return false; }

      if (animationName) {
        const animationData = armatureData.getAnimationData(animationName);
  ...
  generateArmature: function (armatureData) {
    const animation = new dragonBones.Animation();
    // sprite
    const display = new cc.Node();
    display.setCascadeColorEnabled(true);
    display.setCascadeOpacityEnabled(true);
    display.retain();
    // eventDispatcher
    const eventDispatcher = new dragonBones.DBCCEventDispatcher();
  ...
  generateDisplay: function (textureAtlas, textureData, displayData) {
    ...
    const dbccTextureAtlas = textureAtlas;

    if (!dbccTextureAtlas || !textureData) return null;

    const texture = dbccTextureAtlas.getTexture();
    //cc.assert(texture);
    ...
    const spriteFrame = new cc.SpriteFrame(texture, rect, textureData.rotated, offset, originSize);
    const display = new cc.Sprite(spriteFrame);
    display.setCascadeColorEnabled(true);
    display.setCascadeOpacityEnabled(true);
    display.retain();
    let pivotX = 0;
...
dragonBones.DBCCSlot = dragonBones.Slot.extend({
  ...
  getBoundingBox: function () {
    ...
    const displayA = this.getCCDisplay();
    if (displayA) {
      const r = displayA.getBoundingBox();
      return new dragonBones.Rectangle(r.origin.x, r.origin.y, r.size.width, r.size.height);
    }
    return new dragonBones.Rectangle();
  },
  ...
  addDisplayToContainer: function (container, zIndex) {
    if (this._nodeDisplay && container) {
      ...
      const sprite = new cc.Sprite(this._nodeDisplay.texture, this._nodeDisplay._rect);
      if (zIndex < 0) {
  ...
  updateDisplayBlendMode: function (blendMode) {
    const spriteDisplay = this._nodeDisplay;

    if (spriteDisplay) {
      switch (blendMode) {
        case dragonBones.BlendMode.BM_ADD:
          {
            const texture = spriteDisplay.getTexture();
      ...
      if (this._childArmature) {
        let slot;
        const list = this._childArmature.getSlots();
...
dragonBones.DBCCTextureAtlas = cc.Class.extend({
  ...
  getTexture: function () {
    ...
    const textureCache = cc.textureCache;
    let texture = textureCache.getTextureForKey(this.textureAtlasData.imagePath);
    if (!texture) {
